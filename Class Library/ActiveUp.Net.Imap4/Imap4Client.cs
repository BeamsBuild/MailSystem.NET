...
namespace ActiveUp.Net.Mail
{
    ...
    public class Imap4Client : System.Net.Sockets.TcpClient
    {
        ...
        public async void StartIdle()
        {
            ...
            while (true)
            {
                if (_idleInProgress)
                {
                    ...
                    try
                    {
                        response = await TryAwait(sr.ReadLineAsync(), 900000); //15 minutes
                    }
                    catch
                    {
                    ...
                    {
                        this.OnNewMessageReceived(new NewMessageReceivedEventArgs(int.Parse(response.Split(' ')[1]), response));
                    }
#if DEBUG
        ...
        private async Task<T> TryAwait<T>(Task<T> target, int delay)
        {
            var completed = await Task.WhenAny(target, Task.Delay(delay));
            if (completed == target)
            {
                return await target;
            }
